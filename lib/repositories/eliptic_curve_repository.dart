...
class ElipticCurveRepository {
  ...
  final BigInt k = BigInt.parse("0373744f99d31509eb5f8caaabc0cc3fab70e571a5db4d762020723b9cd6ada260", radix: 16);
  ...
  factory ElipticCurveRepository.fromSeed(String seed) {
    ...
    return ElipticCurveRepository(seed: seed, random: secureRandom, curve: curveInit, generator: generatorInit);
  }
  factory ElipticCurveRepository.noSeed() {
    final secureRandom = _initSecureRandom();
    final curveInit = ECCurve_secp256k1();
    final generatorInit  = _initKeyGenerator(curveInit, secureRandom);
    return ElipticCurveRepository(random: secureRandom, curve: curveInit, generator: generatorInit);
  }

  ElipticCurveRepository({this.seed, required this.random, required this.curve, required this.generator});
  ...
  BigInt pointToBigInt(ECPoint point) {
    var bytes = point.getEncoded(true).join();
    return BigInt.parse(bytes, radix: 10);
  }
}
