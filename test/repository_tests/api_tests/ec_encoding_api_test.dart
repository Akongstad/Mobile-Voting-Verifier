...
void main() {
  ...
  final expectedEncodedpoint = params.curve.createPoint(BigInt.parse(
  ...
  group("decodePoint tests", () {
    test('check numbersFromSeedAsync from example', () async {
      final actual = await ECEncodingAPI.decodeFromPoint(expectedEncodedpoint.x!.toBigInteger()!, expectedEncodedpoint.y!.toBigInteger()!);
      expect(actual, expectedDecodedMessage);
    });
  });
  group("encodePoint tests", () {
    test('encodePoint returns expected point given example input', () async {
      final primeP= ECEncodingAPI.specP;
      final actual = await ECEncodingAPI.encodeToPoint(expectedDecodedMessage, primeP, params);
